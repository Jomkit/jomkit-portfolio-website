function ProjectOverlay({ children, header = "", footer = "", url = "#" }) {
  const content = (
    <div className="group relative overflow-hidden rounded border border-slate-200 bg-white dark:border-border dark:bg-card">
      {children}
      {header && (
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="block focus:outline-none focus-visible:ring-2 focus-visible:ring-ring">
      {content}
    </a>
  );
}
